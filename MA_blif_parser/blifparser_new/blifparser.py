...
try:
    ...
    import blifparser_new.utils as utils
            ...
            while line != "":
                ...
                try:
                    if linestrip == "":
                        ...
                        # found a keyword
                        if keyword == ".model":
                            # check if this is main model
                            if(self.blif.model == None):
                                is_model = True
                                self.blif.model = keywords.generic.Model(params)
                            # add new submodel
                            else:
                                is_submodel = True
                                self.blif.submodel.append(keywords.subfiles.Submodel())
                                self.blif.submodel[-1].model = keywords.generic.Model(params)
                                
                        elif keyword == ".inputs":
                            if(is_submodel == False):
                                self.blif.inputs = keywords.generic.Inputs(params)
                            else:
                                self.blif.submodel[-1].inputs = keywords.generic.Inputs(params)
                                
                        elif keyword == ".outputs":
                            if(is_submodel == False):
                                self.blif.outputs = keywords.generic.Outputs(params)
                            else:
                                self.blif.submodel[-1].outputs = keywords.generic.Outputs(params)
                        
                        elif keyword == ".blackbox":
                            self.blif.submodel[-1].blackbox = ".blackbox"
